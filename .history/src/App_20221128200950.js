...
function App() {
  ...
  async function getData(isListing, minPrice, maxPrice, isHome, isRoom, minNights) {
    ...
    minNights=0
      ...
      await axios.get(`http://localhost:8080/neighborhoods`).then((res) => {
      alert(res.data)
    ...
    if (isListing == true) {
      ...
      for (let neighs = 0; neighs < neighborhoods.length; neighs++) {
        ...
        await axios.get(`http://localhost:8080/listings/${neighnames}?minPrice=${minPrice}&maxPrice=${maxPrice}&home=${isHome}&room=${isRoom}&minNights=${minNights}`)
          .then((res) => {
