...
import { useState, useSupercluster,useRef } from 'react';
...
function App() {
  ...
  const lists = data && !error ? data : [];

  const points = lists.map(listing => ({
    type: "Feature",
    properties: { cluster: false, listsId: lists.id, category: lists.category },
    geometry: {
      type: "Point",
      coordinates: [
        parseFloat(lists.location.longitude),
        parseFloat(lists.location.latitude)
      ]
    }
  }));
  ...
  return (
    <div className="App">
      ...
      >
        ...
        // the point may be either a cluster or a lists point
        const {
          cluster: isCluster,
        ...
        return (
          <Marker
            key={`neigh-${cluster.properties.neighId}`}
            latitude={latitude}
            longitude={longitude}
          >
            <button className="neigh-marker">
            </button>
          </Marker>
        );
      })}
