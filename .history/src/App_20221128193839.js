...
function App() {
  ...
  function getData(isListing, minPrice, maxPrice, isHome, isRoom, minNights) {
    ...
    if (isListing = true) {
      ...
      {
        axios.get(`http://localhost:8080/listings/${neighborhoods[neighs]["neighborhood_group"]}?minPrice=${minPrice}&maxPrice=${maxPrice}&home=${isHome}&room=${isRoom}&minNights=${minNights}`)
          .then(res => {
