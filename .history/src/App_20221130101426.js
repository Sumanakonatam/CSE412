...
function App() {
  ...
  const points = listings.map(listings => ({
    type: "Feature",
    properties: { cluster: false, listingsId: listings.id, category: listings.category },
    geometry: {
      type: "Point",
      coordinates: [
        parseFloat(listings.location.longitude),
        parseFloat(listings.location.latitude)
      ]
    }
  }));
    ...
    <div className="App">
      ...
      >
        {clusters.map(cluster => {
          ...
          return (
            <Marker
              key={`listings-${cluster.properties.listingsId}`}
              latitude={latitude}
              longitude={longitude}
            >
              <button className="listings-marker">
                <img src="/custody.svg" alt="listings doesn't pay" />
              </button>
            </Marker>
          );
        })}
