...
function App() {
  ...
  async function getData(isListing, minPrice, maxPrice, isHome, isRoom, minNights) {
    ...
    const neighs = await axios.get(`http://localhost:8080/neighborhoods`)
    alert(neighs.data)
    
    await setNeighborhoods(neighs.data)
    ...
    if (isListing == true) {
      ...
      for (let neighs = 0; neighs < neighborhoods.length; neighs++) {
        ...
        const listings = await axios.get(`http://localhost:8080/listings/${neighnames}?minPrice=${minPrice}&maxPrice=${maxPrice}&home=${isHome}&room=${isRoom}&minNights=${minNights}`)
        alert(listings.data.count)
      }
    }
    ...
    else if(isListing==false)
    {
      ...
      for (let neighs = 0; neighs < neighborhoods.length; neighs++) {
        ...
        const neighnames=neighborhoods[neighs]["neighborhood_group"]
        const hosts = await axios.get(`http://localhost:8080/hosts/${neighnames}`)
        alert(hosts.data.count)
      }
