  ...
  function getData(isListing, minPrice, maxPrice, isHome, isRoom, minNights) {
    ...
    if ((isListing = true)) {
      ...
      for (neighs = 0; neighs < neighborhoods.length; neighs++) {
        neighnames=neighborhoods[neighs]["neighborhood_group"]
        axios
          .get(
            `http://localhost:8080/listings/${neighborhoods[neighs]["neighborhood_group"]}?minPrice=${minPrice}&maxPrice=${maxPrice}&home=${isHome}&room=${isRoom}&minNights=${minNights}`
          )
          .then((res) => {
            listings[]
          });
